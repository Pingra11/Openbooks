...
import { 
  ...
  query, where, orderBy, Timestamp, writeBatch, runTransaction
...
let submissionToken = null; // Idempotency token for current submission
...
/**
 * Sort entries: pending_approval first, then by date descending
 */
function sortEntriesByPriority(entries) {
  return entries.sort((a, b) => {
    ...
    const aDate = a.entryDate?.toDate ? a.entryDate.toDate() : new Date(a.entryDate);
    const bDate = b.entryDate?.toDate ? b.entryDate.toDate() : new Date(b.entryDate);
    return bDate - aDate;
  });
}
...
async function loadJournalEntries(filters = {}) {
  try {
    ...
    displayEntries(sortEntriesByPriority(entries));
...
window.applyFilters = function() {
  ...
  const searchFilter = document.getElementById('searchEntry')?.value || '';
  ...
  if (searchFilter) {
    ...
    filtered = filtered.filter(e => 
      e.entryNumber?.toString().includes(searchLower) ||
      e.description?.toLowerCase().includes(searchLower) ||
      e.reference?.toLowerCase().includes(searchLower) ||
      e.lineItems?.some(item => item.accountName?.toLowerCase().includes(searchLower)) ||
      e.totalAmount?.toString().includes(searchFilter)
    );
  }
  ...
  displayEntries(sortEntriesByPriority(filtered));
};
...
window.clearFilters = function() {
  ...
  displayEntries(sortEntriesByPriority([...allEntries]));
};
...
window.showNewEntryModal = async function() {
  ...
  // Generate new submission token for idempotency
  generateSubmissionToken();
  
  // Attach auto-clear listeners to header fields
  attachJournalAutoClear();
...
window.addLineItem = function() {
  ...
  // Attach auto-clear listeners to the new line item
  attachLineItemAutoClear(lineItemCounter);
...
window.updateTotals = function() {
  ...
  if (difference === 0 && totalDebits > 0) {
    ...
    // Auto-clear balance errors when debits = credits
    clearError(ERROR_CODES.JE_UNBALANCED);
  } else {
    diffElement.style.color = 'var(--error)';
  }
  
  // Auto-clear zero total error if total amount is non-zero
  // Clear when totalDebits + totalCredits > 0 (not both individually)
  if (totalDebits + totalCredits > 0) {
    clearError(ERROR_CODES.JE_ZERO_TOTAL);
  }
  
  // Auto-clear insufficient lines error if we have enough line items with amounts
  ...
  accountSelects.forEach(select => {
    ...
    if (select.value && (debit > 0 || credit > 0)) {
      validLineItems++;
    }
  });
  if (validLineItems >= 2) {
    clearError(ERROR_CODES.JE_INSUFFICIENT_LINES);
  }
};
...
/**
 * Generate a new submission token for idempotency
 */
...
/**
 * Auto-clear validation errors in real-time
 * Attach listeners to form fields to clear errors when fixed
 */
function attachJournalAutoClear() {
  // Clear date error when date is filled
  const dateField = document.getElementById('entryDate');
  if (dateField) {
    dateField.addEventListener('change', () => {
      if (dateField.value) {
        clearError(ERROR_CODES.JE_MISSING_DATE);
      }
    });
  }
  
  // Clear description error when description is filled
  const descField = document.getElementById('entryDescription');
  if (descField) {
    descField.addEventListener('input', () => {
      if (descField.value.trim()) {
        clearError(ERROR_CODES.JE_MISSING_DESCRIPTION);
      }
    });
  }
}
...
/**
 * Attach auto-clear listeners to a specific line item
 */
function attachLineItemAutoClear(lineNumber) {
  ...
  // Clear account/amount errors when line item becomes valid
  const validateLineItem = () => {
    const accountId = accountSelect.value;
    const debit = parseFloat(debitInput.value) || 0;
    const credit = parseFloat(creditInput.value) || 0;
    
    // Clear missing account error if:
    // 1. Account is selected and has amount, OR
    // 2. Both account and amounts are empty (no conflict)
    if ((accountId && (debit > 0 || credit > 0)) || (!accountId && debit === 0 && credit === 0)) {
      clearError(ERROR_CODES.JE_MISSING_ACCOUNT);
    }
    
    // Clear missing amount error if:
    // 1. Account has amount, OR
    // 2. Account is not selected (no conflict)
    if ((accountId && (debit > 0 || credit > 0)) || !accountId) {
      clearError(ERROR_CODES.JE_MISSING_AMOUNT);
    }
    
    // Clear both debit/credit error if only one or neither has value
    if (!(debit > 0 && credit > 0)) {
      clearError(ERROR_CODES.JE_BOTH_DEBIT_CREDIT);
    }
    
    // Clear invalid amount error if amounts are valid or empty
    // Valid means: 0 (empty/cleared) OR >= 0.01
    // This handles NaN/blank inputs as cleared state
    const debitValue = parseFloat(debitInput.value);
    const creditValue = parseFloat(creditInput.value);
    const debitValid = !debitValue || debitValue >= 0.01; // Empty (NaN) or valid
    const creditValid = !creditValue || creditValue >= 0.01; // Empty (NaN) or valid
    if (debitValid && creditValid) {
      clearError(ERROR_CODES.JE_INVALID_AMOUNT);
    }
    
    // Check for inactive account - clear error if no account selected or active account
    if (!accountId) {
      clearError(ERROR_CODES.JE_INACTIVE_ACCOUNT);
    } else {
    ...
    // Check for duplicate accounts across all lines
    refreshDuplicateAccountErrors();
  };
  
  accountSelect.addEventListener('change', validateLineItem);
  debitInput.addEventListener('input', validateLineItem);
  creditInput.addEventListener('input', validateLineItem);
}

/**
 * Check all line items and clear duplicate account error if no duplicates exist
 */
function refreshDuplicateAccountErrors() {
  const accountSelects = document.querySelectorAll('.account-select');
  const accountIds = Array.from(accountSelects)
  ...
  if (duplicates.length === 0) {
    clearError(ERROR_CODES.JE_DUPLICATE_ACCOUNT);
  }
}
...
/**
 * Helper function to disable/enable submit buttons during processing
 */
function disableSubmitButtons(disable) {
  const submitButtons = document.querySelectorAll('[onclick*="saveJournalEntry"]');
...
window.saveJournalEntry = async function(status) {
  try {
    ...
    // Idempotency check: prevent concurrent submissions (after validation)
    if (isSubmitting) {
      return;
    }
    
    // Set submitting flag and disable all buttons
    isSubmitting = true;
    ...
    const entryData = {
      ...
      submissionToken: entryId ? null : submissionToken // Only for new entries
    };
    ...
    if (entryId) {
      ...
      // Idempotency check: submission token must exist (fail hard if missing)
      if (!submissionToken) {
        console.error('❌ No submission token found - this should never happen');
        isSubmitting = false;
        disableSubmitButtons(false);
        alert('Error: Invalid submission state. Please close and reopen the form.');
        return;
      }
      ...
      // Atomic idempotency: use submissionToken as document ID with transaction
      const entryRef = doc(db, "journalEntries", submissionToken);
      ...
      const created = await runTransaction(db, async (transaction) => {
        ...
        if (entryDoc.exists()) {
          // Entry already exists with this token (idempotent success)
          return false;
        }
        
        // Entry doesn't exist, create it atomically
        transaction.set(entryRef, entryData);
        return true;
      });
      
      if (!created) {
        // Duplicate detected, idempotent success
        isSubmitting = false;
        disableSubmitButtons(false);
        closeJournalModal();
        loadJournalEntries();
        alert('This entry was already submitted successfully.');
        return;
      }
      
      const newEntryId = submissionToken;
      
      // If posting directly, update account balances
      if (status === 'posted' && isManager) {
        await postJournalEntry(newEntryId, lineItems, reference);
      }
    }
    ...
    if (status === 'pending_approval') {
      try {
        ...
        const response = await fetch('/api/notify-managers-journal-submission', {
          method: 'POST',
          headers: {
    ...
    // Clear submitting flag and re-enable buttons
    isSubmitting = false;
    disableSubmitButtons(false);
    ...
    // Clear submitting flag and re-enable buttons on error
    isSubmitting = false;
    disableSubmitButtons(false);
...
window.editEntry = async function(entryId) {
  try {
    ...
    // Attach auto-clear listeners to all existing line items
    document.querySelectorAll('.account-select').forEach(select => {
      const lineNum = select.dataset.line;
      if (lineNum) {
        attachLineItemAutoClear(parseInt(lineNum));
      }
    });
