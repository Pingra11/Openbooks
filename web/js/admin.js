      ...
      tr.innerHTML = `
        ...
        <td>
          <div class="user-actions">
            <button onclick="approveRequest('${docSnap.id}')" class="btn-activate" title="Approve this request">Approve</button>
            <button onclick="rejectRequest('${docSnap.id}')" class="btn-suspend" title="Reject this request">Reject</button>
          </div>
        </td>
      `;
...
async function loadUsers() {
  try {
    ...
    // Sort users by role (Administrator > Manager > Accountant), then by name
    const roleOrder = { 'administrator': 1, 'manager': 2, 'accountant': 3 };
    allUsers.sort((a, b) => {
      const aRoleOrder = roleOrder[a.role?.toLowerCase()] || 99;
      const bRoleOrder = roleOrder[b.role?.toLowerCase()] || 99;
...
window.saveEditedUser = async function(event) {
  ...
  try {
    ...
    // Check if name changed - if so, regenerate username
    let newUsername = originalUser.username;
    if (firstName !== originalFirstName || lastName !== originalLastName) {
      newUsername = generateUsername(firstName, lastName);
      console.log(`Name changed, regenerating username: ${originalUser.username} -> ${newUsername}`);
    }
    
    // Update Firebase Authentication FIRST if email changed - MUST happen before Firestore
    if (email !== originalEmail) {
      try {
        // Use proxy endpoint through main server to avoid CORS issues
        const adminServerUrl = '/api/update-firebase-user';
    ...
    // CRITICAL: If username changed, update the usernames collection for login
    if (newUsername !== originalUser.username) {
      console.log(`Updating usernames collection: ${originalUser.username} -> ${newUsername}`);
      
      // Delete old username mapping
      await deleteDoc(doc(db, "usernames", originalUser.username));
      
      // Create new username mapping with the user's email
      ...
      console.log(`Username mapping updated successfully`);
    }
    
    // If email changed but username didn't, update the email in usernames collection
    if (email !== originalEmail && newUsername === originalUser.username) {
      await updateDoc(doc(db, "usernames", newUsername), {
        email: email
      });
      console.log(`Updated email in usernames collection for ${newUsername}`);
    }
...
async function loadRecentActivity() {
  try {
    ...
    if (activitySnap.empty) {
      activityDiv.innerHTML = '<div class="empty-state"><p>No recent activity</p></div>';
      return;
    }
...
window.saveSecurityQuestions = async function(event) {
  ...
  try {
    ...
    // Get form values - use option text for question display
...
Type DELETE in the next prompt to confirm deletion.`);
    ...
    try {
      // Use API endpoint for user deletion
  ...
  if (user) {
    ...
    if (userDoc.exists() && userDoc.data().role === "administrator") {
      ...
      await Promise.all([
        ...
        loadRecentActivity(),
        loadFinancialRatios(),
        loadImportantMessages('administrator')
      ]);
    } else {
